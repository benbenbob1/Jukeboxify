...
module.exports = {
    ...
    searchTrack: function(query, callback) {
    ...
    addSongToVotePool: function(song) {
        var songFound = false;
        for (var i=0; i<votePool.length; i++) {
    ...
    printTopSongsByVotes: function(numToShow=10) {
        var output = [];
    ...
    voteByIndex: function(zeroBasedIndex, voteDir=1) {
    ...
    removeSongByIndex: function(zeroBasedIndex) {
        votePool.splice(zeroBasedIndex, 1);
    },

    // 1 or -1 to vote
    voteBySongId: function(songId, voteDir=1) {
    ...
    getSongFromSpotifyId: function(songId, callback) {
    ...
    addSongToPoolById: function(songId) {

        return addSongToVotePool(new Song(songId));
    },

    sortVotePoolByVotes: function(pool) {
        pool.sort(function(songA, songB) {
            return songB.vote - songA.vote;
        })
    },

    getTopVotes: function(numToShow=10) {
